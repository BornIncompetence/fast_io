...
auto main() -> int {
    {
        auto out = fast_io::c_file("cfile.txt", fast_io::open_mode::binary | fast_io::open_mode::out);
    ...
    {
        auto in = fast_io::c_file("cfile.txt", fast_io::open_mode::binary | fast_io::open_mode::in);
        const auto duration = bench([&](std::uint64_t i) {
            scan(in, v[i]);
        });
